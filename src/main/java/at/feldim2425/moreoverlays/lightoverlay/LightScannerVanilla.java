...
public class LightScannerVanilla extends LightScannerBase {
	...
	public byte getSpawnModeAt(BlockPos pos, World world) {
		if (world.getLightFor(LightType.BLOCK, pos) >= Config.light_SaveLevel)
			return 0;
		...
		if (world.getLightFor(LightType.SKY, pos) >= Config.light_SaveLevel)
			return 1;
	...
	private static boolean checkCollision(BlockPos pos, World world) {
		...
		if (block1.isNormalCube(world, pos) || (!Config.light_IgnoreLayer && world.getBlockState(pos.up()).isNormalCube(world, pos.up()))) //Don't check because a check on normal Cubes will/should return false ( 99% collide ).
			return false;
